...
Future<Response> ioRequester(Request req) {
  ...
  return _client.openUrl(req.method, req.uri).then((HttpClientRequest request) {
    ...
    res.headers = headers;
    return response.fold([], (List body, next) => body..addAll(next)).then(
        (data) => res..body = data.cast<int>());
  });
}
